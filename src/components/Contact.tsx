import { ArrowUpRight, Check, Copy, Github, Linkedin, MessageSquare, Send } from "lucide-react";
...
export function Contact() {
  ...
  const handleCopyEmail = (e: React.MouseEvent) => {
    e.preventDefault();
    navigator.clipboard.writeText("contact@mchacon.dev");
    setCopied(true);
    ...
    >
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-[880px]:gap-3 pb-8 max-[880px]:pb-60">
          ...
          <div
            className="md:col-span-2 group relative overflow-hidden rounded-xl bg-gradient-to-br from-primary/20 to-primary/5 border border-text-primary/10 p-6 transition-all duration-300 ease-out hover:bg-text-primary/10 hover:border-primary/30 hover:shadow-lg hover:-translate-y-1 active:scale-[0.98] animate-in fade-in slide-in-from-bottom-8 max-[880px]:p-5"
            style={{ animationDelay: "150ms" }}
          >
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 relative z-10">
              <div className="max-w-md">
                <h3 className="text-xl font-bold text-text-primary mb-1 max-[880px]:text-lg">
                  {t("contact.cardTitle")}
                </h3>
                <p className="text-text-secondary text-sm max-[880px]:text-xs">
              ...
              <div className="flex gap-2 w-full md:w-auto max-[880px]:flex-row">
                <a
                  href="mailto:contact@mchacon.dev"
                ...
                >
                  ...
                  <Send
                    size={18}
                    className="relative z-20 group-hover/mail:translate-x-0.5 group-hover/mail:-translate-y-0.5 transition-transform duration-500"
                  />
                  <span className="relative z-20 max-[880px]:text-sm whitespace-nowrap">
                    {t("contact.sendEmail")}
                  </span>
                </a>
          ...
          >
            {/* Icono */}
            <div
              className="p-2.5 rounded-xl bg-gradient-to-br from-[var(--color-linkedin,#0a66c2)]/20 to-[var(--color-linkedin,#0a66c2)]/10 border border-[var(--color-linkedin,#0a66c2)]/20 transition-all duration-300 relative z-10 max-[880px]:shrink-0 max-[880px]:mr-3"
              style={{ color: "var(--color-linkedin, #0a66c2)" }}
            >
            ...
            <div className="relative z-10 mt-2 max-[880px]:mt-0 max-[880px]:flex-1">
              <h3 className="text-lg font-bold text-text-primary mb-1 max-[880px]:text-sm max-[880px]:mb-0.5">
                {t("contact.linkedinTitle")}
              </h3>
              <p className="text-text-secondary text-xs max-[880px]:text-[10px]">
                {t("contact.linkedin")}
              </p>
            </div>
            ...
            <div className="ml-auto max-[880px]:ml-0 max-[880px]:shrink-0">
              <ArrowUpRight
                size={18}
                className="text-text-secondary group-hover:text-text-primary transition-all duration-300 max-[880px]:w-4 max-[880px]:h-4"
              />
            </div>
          </a>
