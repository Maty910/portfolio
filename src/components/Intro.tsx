  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center gap-8">
        ...
        <div
          className={`relative transition-all duration-700 delay-100 ${isExiting ? "translate-y-[-20px] opacity-0" : "translate-y-0 opacity-100"}`}
        >
        ...
        <div className="text-center space-y-3 overflow-hidden">
          <h1 className="text-4xl md:text-5xl font-bold text-text-primary tracking-tighter flex flex-col items-center">
            <span className="block overflow-hidden h-[1.3em]">
              <span
                className={`block animate-slide-up-reveal delay-300 ${isExiting ? "translate-y-[100%]" : ""}`}
              >
                {t("intro.name").split(" ")[0]}{" "}
                <span className="text-primary">{t("intro.name").split(" ")[1]}</span>
              </span>
            </span>
          </h1>
          ...
          <div className="flex items-center gap-2 justify-center text-text-secondary text-xs font-mono tracking-[0.2em] uppercase">
            ...
            <span className="opacity-0 animate-[fade-in_0.5s_ease-out_0.5s_forwards]">
              {t("intro.role")}
            </span>
          </div>
        </div>
